...
class PrimeSieve:
    ...
    def is_prime(n):
        ...
        if n == 2:
            return True
        if n >= 3 and n % 2 == 1:
            for div in range(3, int(math.ceil(n ** 0.5)) + 1, 2):
                if n % div == 0:
                    return False
            return True
        return False
    ...
    def prime_comprehension(self) -> list[int]:
        ...
        if self.n <= 2:
            return []
        return [number for number in [2, *range(3, self.n, 2)] if PrimeSieve.is_prime(number)]
...
def profile_small_input_many_repetitions(func: Callable[[], Iterable[int]]) -> None:
...
def profile_big_input(func: Callable[[], Iterable[int]]) -> None:
...
def main():
    ps = PrimeSieve(1_000_000)
    ps.prime_functional()
    ps.prime_imperative()
    ps.prime_comprehension()
