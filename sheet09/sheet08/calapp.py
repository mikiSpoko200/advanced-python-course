...
import db
import ui
from ui import ParamInfo, Config
...
class Event(IComponent, IUIComponent):
    ...
    def args(self) -> list[ArgumentInfo]:
        """Override default implantation since we need to include relations."""
        base_data = model_arg_info(Event)
        host = model_arg_info(Participant)
...
class EventType(IComponent, IUIComponent):
    ...
    @property
    def query_args(self) -> dict[db.pw.Field: DBValue]:
        return self._query_args


DBValue = str | datetime | Participant | EventType | Optional[str] | list[Participant]
